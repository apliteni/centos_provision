...
declare -A REPLAY_ROLE_TAGS_SINCE=(
  ...
  ['install-clickhouse']='2.38.2'
  ['install-mariadb']='2.38.2'
  ['install-redis']='2.38.2'
...
download_kctld(){
  run_command "curl -fsSL ${KCTLD_URL} | tar -xzC ${KCTL_BIN_DIR}" "Downloading kctld" "hide_output"
}
...
ExecStartPre=-/usr/bin/bash -c \\
              "(/usr/bin/podman ps | /usr/bin/grep -qw nginx) \\
                && /usr/bin/podman rm --force --volumes nginx"
...
install_nginx_on_docker() {
  ...
  if file_content_matches "${PATH_TO_NGINX_ROADRUNNER_CONFIG}" "-F" "tracker.status"; then
    sed -i "/tracker.status/d" "${PATH_TO_NGINX_ROADRUNNER_CONFIG}"
  fi
