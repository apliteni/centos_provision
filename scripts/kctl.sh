...
kctl_renew_certificates() {
  ...
  command="/usr/bin/docker run -it --rm $(dockerized_certbot_volumes) certbot/certbot renew ${renew_args}"
  LOG_PATH="${log_path}"
  init_log "${log_path}"

  debug "Renewing certificates"
  if run_command "${command}" "" 'hide_output' 'allow_errors' > /dev/null; then
