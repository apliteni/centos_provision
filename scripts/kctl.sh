...
kctl_features.set_olap_db() {
  ...
  kctl_features.assert_tracker_supports_rbooster
...
kctl_features.enable_rbooster() {
  ...
  kctl_features.run_ch_migrator
  kctl_features.start_running_tracker_tasks
}

kctl_features.start_running_tracker_tasks() {
  rm -f "${PATH_TO_TRACKER_UPDATE_LOCK}"
}
...
kctl_features.stop_running_tracker_tasks() {
  touch "${PATH_TO_TRACKER_UPDATE_LOCK}"
  wait_for_complete_cron_php
}
...
kctl_features.run_ch_migrator(){
  ...
  run_command "TZ=UTC kctl-ch-migrator --prefix=${prefix} --env-file-path=${INVENTORY_DIR}/tracker.env"
}
...
PATH_TO_TRACKER_UPDATE_LOCK="${TRACKER_ROOT}/var/locks/update.lock"
