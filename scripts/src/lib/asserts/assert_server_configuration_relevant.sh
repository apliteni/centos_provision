...
assert_server_configuration_relevant(){
  ...
  if isset "$SKIP_CHECKS"; then
    ...
    if [[ "${RELEASE_VERSION}" == "${installed_version}" ]]; then
      ...
      command="curl -fsSL https://keitaro.io/release-${installed_version}/${TOOL_NAME}.sh | bash -s -- -wa ${TOOL_ARGS}"
      if run_command "${command}" "Run obsolete ${TOOL_NAME}" "" "allow_errors"; then
